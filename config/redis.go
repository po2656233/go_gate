...
func RedisHandle() *redis.Client {
	once.Do(func() {
		redisClient = redis.NewClient(&redis.Options{
			Addr:     GlobalXmlConfig.Options.RedisInfo.Address,
			Password: GlobalXmlConfig.Options.RedisInfo.Password, // no password set
			DB:       GlobalXmlConfig.Options.RedisInfo.DBNum,    // use default DB
		})
		pong, err := redisClient.Ping().Result()
		log.Debug(":%v :%v", pong, err)
		if nil != err {
			//self.DB.essence = nil //允许不使用redis
			log.Fatal("cann't to connect redis:%v  error:%v", GlobalXmlConfig.Options.RedisInfo.Address, err)
		}
	})
	return redisClient
}
